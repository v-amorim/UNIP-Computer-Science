\documentclass[12pt, a4paper]{article} % Tamanho da fonte, formato do papel e tipo de documento
%----------------------------------------------------------------------------------------
%	Packages
%----------------------------------------------------------------------------------------
\usepackage{fontspec} % Define a fonte
\usepackage[portuguese]{babel} % Idioma Português
\usepackage{listings} %Define as listas (numeradas)
\usepackage{tikzsymbols} % Símbolos matemáticos
\usepackage{geometry} % Define margens
\usepackage{setspace} % Define os espaçamentos
\usepackage{fancyhdr} % Necessário para header e footer
\usepackage{titlesec} % Define um ponto após a numeração de secções
%----------------------------------------------------------------------------------------
% Define a fonte em Arial
\setmainfont{Arial} 

% Define as listas (numeradas)
\lstset{numbers=left, stepnumber=1, firstnumber=1, numberstyle=\tiny, extendedchars=true,
breaklines=true, frame=tb, basicstyle=\footnotesize, stringstyle=\ttfamily, showstringspaces=false}
...
% Define um ponto após a numeração de secções
...
\newpage
\section{Introdução}
\qquad O foco principal do trabalho é desenvolver um programa de login com criptografia \textbf{3DES} utilizando a linguagem \textbf{PYTHON}, muito utilizado no cenário atual, grandes empresas trocam informações sigilosas diariamente e devido a isso é extremamente importante manter seus dados seguros. 
...
\subsection{O que é criptografia?}
\qquad A criptografia é uma prática utilizada para proteger dados de quaisquer tipos de invasões, ela faz a utilização de codificações onde um algoritmo é responsável em mudar os dados, para que eles não possam ser lidos e somente com uma chave específica esses dados voltam ao seu formato original.
...
\qquad A criptografia tem uma ampla variedade de tipos e cada uma tem seu próprio diferencial, tanto como a aplicação ou como sua dificuldade de utilização. Aqui iremos citar alguns dos tipos mais conhecidos e utilizado atualmente.
...
%----------------------------------------------------------------------------------------
\newpage
\subsubsection{Criptografia simétrica ou chave privada}

\qquad A criptografia simétrica ou chave privada ele trabalha na utilização de uma única chave onde essa chave é compartilhada entre o emissor e o destinatário que as guarda de forma sigilosa. Essa chave vai definir como o algoritmo vai cifrar o conteúdo utilizando uma cadeia própria de bits.
...
%----------------------------------------------------------------------------------------
\subsubsection{Criptografia assimétrica ou chave pública}
...
\qquad Com a facilidade em burlar a criptografia DES houve a necessidade da criação de outro tipo de proteção que seja mais forte contra os hackers, assim foi criado o Triple DES.
...
\subsubsection{Histórico do 3DES}

\qquad O intuito da criação desse tipo de criptografia foi para aprimoração da segurança de sua utilização, ela é uma versão mais segura do Data Encryption Standard e foi criada no ano de 1974 pela empresa International Business Machines Corporation (IBM), uma empresa do ramo de informática e tecnologia, porém sua utilização se tornou padrão somente três anos depois.

\subsubsection{Descrição da técnica}

\qquad A técnica utilizada pelo 3DES se baseia na utilização de tres chaves criptográficas, a primeira chave faz a criptografia da mensagem, a segunda faz a decifração da mensagem criptografada e a última torna a criptografia o resultado da operação.

%----------------------------------------------------------------------------------------
\newpage
\subsection{Software Construído}

\qquad O software é responsável por controlar a entrada de pessoas em uma determinada área privada, cada pessoa autorizada terá um usuário e senha que serão criados e inseridos dentro do Banco de Dados, essas senhas serão criptografadas utilizando a técnica de criptografia 3DES. No algoritmo temos uma chave "Sixteen byte key" que possui 16 bits.
...
\lstinputlisting[language=python, label=acessar, caption={acessar\_bd.py}]{codigos/acessar_bd.py}
\newpage
\lstinputlisting[language=python, label=acessarGUI, caption={APS\_acessar\_BD.py}]{codigos/acessar_bd.py}
\newpage
\lstinputlisting[language=python, label=criar, caption={criar\_usuario\_bd.py}]{codigos/criar_usuario_db.py}
\newpage
\lstinputlisting[language=python, label=criarGUI, caption={APS\_criacao\_BD.py}]{codigos/APS_criacao_BD.py}
